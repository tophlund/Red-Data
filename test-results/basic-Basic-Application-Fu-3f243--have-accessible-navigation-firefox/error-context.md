...
- Name: Basic Application Functionality >> should have accessible navigation
- Location: C:\Red Data\e2e\basic.spec.ts:19:3

# Error details

```
Error: expect.toBeVisible: Error: strict mode violation: locator('a[href="#main"]') resolved to 2 elements:
    1) <a href="#main" class="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 bg-primary-600 text-white px-4 py-2 rounded-md z-50">Skip to main content</a> aka getByRole('link', { name: 'Skip to main content' }).first()
    2) <a href="#main" class="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 bg-primary-600 text-white px-4 py-2 rounded-md z-50">Skip to main content</a> aka getByRole('link', { name: 'Skip to main content' }).nth(1)

Call log:
  - expect.toBeVisible with timeout 5000ms
  - waiting for locator('a[href="#main"]')

    at C:\Red Data\e2e\basic.spec.ts:30:30
```
